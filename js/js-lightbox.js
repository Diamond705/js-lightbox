...
//Determine arrow behaviour based on number of media items
    ...
    $('body').on('click', '.expanded-right, .expanded-left', function(e){
        if($(this).hasClass('disabled')) return;
        var curIndex = $('.expanded-carousel.active').data('index');
        var nextItemIndex = parseInt(curIndex+1);
        if($(this).hasClass('expanded-left')){
            nextItemIndex-=2;
        }
        var nextItem = $('.expanded-carousel[data-index='+nextItemIndex+']');
     ...
     //Declare data variables
        ...
        if ($('.expanded-wrapper').length === 0) {
            if(typeof imagepath !== 'undefined') {
                ...
                modalHtml += "<span class='expanded-modal-location'>"+location+"</span>";
